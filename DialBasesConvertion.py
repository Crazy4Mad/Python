...
def window():
	...
	seven_ten = Button(text="7->10", width = bwidth, height = bheight,
					  command = lambda dial = input_field, out = output_field, base = 7:
	...
	ten_seven= Button(text="10->7", width = bwidth, height = bheight,
					 command = lambda dial = input_field, out = output_field, base = 10:
					  result(base, dial, out)).grid(row = 2, column = 2)
	
	clear = Button(text="clear", width = bwidth, height = bheight, command = lambda inp = input_field, out = output_field:delete(inp, out))
	clear.grid(row = 2, column = 1)
	dot = Button(text=".", width = bwidth, height = bheight,
				 command = lambda f = '.', inp = input_field: calc(f, inp)).grid(row = 6, column = 0)
	minus = Button(text="-", width = bwidth, height = bheight,
				  command = lambda f = '-', inp = input_field: calc(f, inp)).grid(row = 6, column = 2)
